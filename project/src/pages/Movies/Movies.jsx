...
const Movies = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      const movieItems = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })).filter(item => item.type === 'movie');
  ...
  return (
    <div className="pt-20 px-4 space-y-8">
      <ContentRow title="All Movies" type="movie" />
      {categories.map(category => (
        <ContentRow key={category} title={category} category={category} type="movie" />
      ))}
    </div>
  );
};
