...
const Movies = () => {
  ...
  useEffect(() => {
    if (!loading && subscriptionStatus === 'active') {
      const fetchMovies = async () => {
        ...
        const movieItems = snapshot.docs
  ...
  if (subscriptionStatus !== 'active')
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-white text-center px-4">
        ...
        <p className="mt-4 text-lg sm:text-xl text-gray-400">You need an active subscription to view Movies.</p>
        <button
          ...
          {filteredMovies.map(movie => (
            ...
            >
              <div
                ...
                onClick={() => navigate(`/details/${movie.id}`)} // ✅ FIXED to go to Details page
              >
                <img
                ...
                <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 flex flex-col justify-end p-3 space-y-2 transition-opacity duration-300">
                  <div className="flex justify-between text-2xl">
                    <span
                      onClick={e => { e.stopPropagation(); handleAddToList(movie.id) }}
                    ...
                    <span
                      onClick={e => { e.stopPropagation(); handleToggleLike(movie.id) }}
                      className="cursor-pointer"
                    >
                      {userLikes[movie.id] === true
                        ? <FaHeart className="text-red-600" />
                        : <FaRegHeart className="text-white" />}
                    </span>
                    <span
                      onClick={e => { e.stopPropagation(); handleToggleDislike(movie.id) }}
                      className="cursor-pointer"
                    >
                      {userLikes[movie.id] === false
  ...
  return (
    <div className="pt-20 px-4">
      <ShowRow title="All Movies" />
      <ShowRow title="Trending" filter={movie => movie.category === 'Trending'} />
      <ShowRow title="Action" filter={movie => movie.category === 'Action'} />
      <ShowRow title="Comedy" filter={movie => movie.category === 'Comedy'} />
    </div>
  )
}
