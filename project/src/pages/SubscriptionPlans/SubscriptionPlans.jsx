  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen text-white bg-black">
      <div className="bg-gray-900 p-8 rounded-lg shadow-lg w-full max-w-2xl text-center">
        ...
        <p className="mb-8 text-gray-400">Select a plan to start watching.</p>

        {loginPrompt && (
          <div className="bg-red-600 text-white p-3 rounded-md mb-4 text-center">
            Please log in to select a plan.
            <button onClick={() => navigate('/login')} className="ml-4 underline font-bold">Go to Login</button>
          </div>
        )}

        <div className="flex flex-col md:flex-row gap-8 justify-center">
          {plans.map((plan) => (
            <div key={plan.id} className="bg-gray-800 p-6 rounded-lg shadow-md flex-1 flex flex-col items-center">
